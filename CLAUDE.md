...
- 2026-06-08: Added Voice Recorder section to SettingsPanel with on/off toggle, duration selector (1-24 hours), list of recordings (with delete and date sort ascending/descending), save button. Created new `src/features/voiceRecorder/` feature with types, service for persistence/background-task scheduling (using expo-av for recording). Recordings saved to document directory and listed in settings. Background task registered for periodic segments (works when app closed via existing expo-background-task setup). Decision: full continuous mic listening uses scheduled segments to respect OS limits; native foreground service for true always-on deferred to later. Updated App.tsx to manage voice settings state/sync.
- 2026-06-08: Added audio playback to voice recordings. Extended `voiceRecorderService.ts` with `playVoiceRecording` using `expo-av` `Sound.createAsync` (auto-unloads on finish). Added "Play" button next to "Delete" in `VoiceRecorderSettingsSection.tsx` (using secondary variant and new `play` icon). Extended `Icon.tsx` with `play` icon. Updated types with playback status. Typecheck passes. Feature allows users to get/list/delete/play audio recordings from settings.
- 2026-06-08: Added Groq Whisper transcription (auto + explicit button) for voice recordings. Text saved to recording metadata, auto-added to VOICENOTES category via noteMutations, editable TextInput below each row with Copy/Save-to-Notes buttons. Extended voiceRecorderTypes, Service (fetch to api.groq.com with FormData), and SettingsSection UI (with useNotesSync for commit). Typecheck clean. Updated histories.

- 2026-06-08: Per user request, transcription triggers as soon as new voice recording files appear in settings (on-demand button + auto on untranscribed recordings via refresh). Fixed to use valid Whisper model (`whisper-large-v3-turbo`) regardless of .env LLM value. Uses EXPO_PUBLIC_GROQ_API_KEY from .env. Results populate editable text dialog below recording with Copy and Save-to-Notes (adds to VOICENOTES) buttons. Updated histories, typecheck clean.

- 2026-06-08: Executed mandatory git commit + push workflow for the completed voice recorder transcription feature. `git status` clean, `git add .`, commit, and `git push -u origin main` completed successfully.
