...
- 2026-07-06: **FIREBASE AUTH MIGRATION & OFFLINE PERSISTENCE**: Connected the app to Firebase project `secondbrain-3c7ac` via `.env` and `env`. Configured Firestore local cache persistence in `firebase.ts`. Created `userRepository.ts` to automatically populate user profiles at `users/{uid}` on signup and update `lastLogin` on signin. Verified typecheck compiles cleanly.
- 2026-07-06: **PHASE 8 MULTI-USER: USER-PREFIXED LOCAL CACHE KEYS**. Refactored `localNotesRepository.ts` and `localAiReviewRepository.ts` to prefix all AsyncStorage keys with `rnnotetaking.u.{uid}` (or `rnnotetaking.anon` for unauthenticated state), fully isolating offline caches per user. Added one-time migration logic to promote existing anonymous-prefix keys to the scoped format on first sign-in. Updated `useNotesSync.ts` and `useAiReviewSync.ts` to thread `uid` into every local read/write call. Refined `clearAllLocalRepositories(uid?)` to wipe only the current user's keys on logout. Typecheck passes with 0 errors.
- 2026-07-06: **PHASE 7 MULTI-USER: SIGN-IN/SIGN-UP/RESET UI FLOWS IMPLEMENTED**. Rewrote `AuthGate.tsx` to handle authentication states using Firebase Auth API (`signInWithEmailAndPassword`, `createUserWithEmailAndPassword`, `sendPasswordResetEmail`). Built styled Notion-like Sign In, Sign Up, and Password Reset screens utilizing existing token design systems (`TextInputField`, `Button`, colors, shadows). Gated the entire app lifecycle behind Firebase user session validation first, keeping the secondary passcode lock as a local device relaunch unlocker. Integrated global context logouts from `AuthProvider` directly.
...
- 2026-06-22: **OCR VISIBLE UI EXPOSURE COMPLETE**. Added OCR modal state + `handleSaveOcrText` in `App.tsx`. Mounted `<OcrModal>` near other modals. Wired `onOpenOcr={() => openOcrModal()}` to `SettingsPanel`. Added `onScanText` to `ActionGrid` props + button (visible in category detail). Settings OCR Import section + Category "Scan Text" button now visible and functional. Typecheck clean after icon fixes (`camera` icon). Git workflow executed.
...
- 2026-06-16: **CHATPTUI SERVER INTEGRATION COMPLETE**. Replaced localhost-only LOCAL_BRIDGE fallback in `requestAiText()` (aiReviewService.ts) with remote ChatPTUI server job-based async API at `http://vmi3321442.tailb6229f.ts.net:8787`. POST `/api/text` with `{ prompt, wait: false }` + `X-API-Key: dev-local-api-key` creates job; polls GET `/api/jobs/{jobId}` until `status === "completed"`; returns only `job.textResponse`. 60s timeout with descriptive errors. Typecheck passes. Both AI Review and AI Chat flows benefit. Git workflow executed.
- 2026-06-17: **TELEPROMPTER SLIM STATUS-BAR REDESIGN**. Redesigned overlay ticker to production-quality slim status-bar style: 28dp height positioned below real status bar using robust height detection (WindowInsets → system resource → density fallback). Removed all extra labels from overlay/notification. Overlay uses FLAG_NOT_TOUCHABLE. Matches exact user requirements. Git workflow executed.
- 2026-06-17: **TELEPROMPTER PERMISSION DIALOG ADDED**. Created TeleprompterPermissionModal.tsx with user-friendly permission request dialog. When clicking Start without overlay permission, shows modal explaining requirement with "Open Settings" button. After granting in Android Settings, user returns and retries successfully. Improved error messages show actual errors. Added ADB debug scripts. Git workflow executed.
...
- 2026-06-13: **EXPOSED REAL FIRESTORE ERROR IN NATIVE CODE**. Popup from Kotlin catch blocks (OverlayService.submitInput, NoteWidgetConfigureActivity.submitNote). Updated to Log full e + Toast with e.message (per TJ debug advice to expose error.code/message first). Rules permissive, local.properties has FIREBASE keys for native REST. Debug plan created in session/plan.md. Rebuild with clean + run-android next to surface the real error (likely permission-denied or not-found). Matches user request exactly. Git workflow next.
- 2026-06-11: **VOICE RECORDER AUDIT & FIX COMPLETE**. 7 issues fixed: (1) Save To - CategoryPicker modal opens, user selects category, saves via addNote/commit. (2) Delete - confirmation per recording, deleteVoiceRecording removes audio/metadata/transcription. (3) Delete Selected - bulk delete with confirmation already works. (4) Playback Bug - root cause was global isPlaying state; fixed with per-recording currentlyPlayingId check. (5) Play/Pause - only matching recording shows Pause. (6) Layout - 4 fixed 80px buttons in actionContainer. (7) Typecheck passes. Updated plan.md/CLAUDE.md. Git workflow executed.
- 2026-06-11: **VOICE RECORDER ACTION CONTROLS LAYOUT FIXED**. Root cause: audioControlsRow used dynamic button widths (playPauseButton: 100px, deleteButton: 52px) + transcription actions wrapped below, causing position shifts. Fixed: single actionContainer with flexDirection:'row', justifyContent:'space-between', width:'100%'; all 5 controls use fixed 80px buttons (fixedButton + deleteButton styles); removed flexWrap/dynamic widths; transcription renders below action row only when present; recordingRow changed to flexDirection:'column'. Layout: [✓] [Play/Pause] [Copy] [Save To] [Delete] - always one horizontal line. Updated plan.md/CLAUDE.md. Git workflow executed.
