...
- 2026-06-22: **OCR FEATURE REINTRODUCED AS ISOLATED FEATURE**. Decision: implement on-device ML Kit OCR as new `src/features/ocr/` boundary. Main Firestore notes document remains AI-clean: only final user-approved OCR text saved as normal note string. No OCR metadata, image URI, bounding boxes, confidence values, or Tesseract artifacts stored inside `reactnativecollection/main.data`. Tesseract and old OCR import behavior remain excluded. Verification wording updated: "Confirm no legacy OCR/Tesseract import path is reintroduced; only new isolated ML Kit OCR feature is allowed." Git workflow next.
