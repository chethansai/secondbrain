...
- 2026-06-11: **TELEPROMPTER IMPLEMENTATION IN PROGRESS**. Root cause analysis: (1) Teleprompter stops on app close - TeleprompterService uses START_STICKY + BootReceiver (boot only), showTeleprompter() silently catches addView() exceptions with wrong "already added" assumption. (2) No category dialog - feature never existed in any layer. (3) No filtering - startTeleprompter() accepts single text string, no category awareness. (4) Persistence incomplete - no categories field in State. Implementation started: TeleprompterSettings.kt added KEY_CATEGORIES + State.selectedCategories + JSON serialization. TeleprompterService.kt extracts categories from intent. OverlayModule.kt accepts ReadableArray categories. React Native layer pending. Git workflow executed.
