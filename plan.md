...
- 2026-06-08: Per user request, ensured transcription triggers as soon as new voice recording files appear in the settings section (auto on load for untranscribed recordings where possible, plus explicit button). Fixed transcription to always use a valid Whisper model (`whisper-large-v3-turbo`) independent of the LLM model in .env. Transcription results populate the editable text box below each recording; edit, copy, and "Save to Notes" (to VOICENOTES) options are present. Groq key from .env is used. Typecheck and git workflow completed.

- 2026-06-08: Re-read plan.md and CLAUDE.md for voice recording. Confirmed existing implementation already includes Pause and Stop buttons for each audio clip (shared global sound in service), bulk select-all with checkboxes, multi-delete with cautionary confirmation modal/overlay. No further code changes needed. `npm run typecheck` clean. Updated histories.
