...
- 2026-07-07: **FIXED ENV QUOTE CONFLICTS**: Resolved `auth/network-request-failed` and client offline errors by removing wrapping double quotes from values in `.env` and defensively adding runtime quote stripping in `firebase.ts` (getFirebaseEnv). Verified strict typechecking passes with 0 errors.
- 2026-07-07: **REMOVED PASSCODE LOCK & IMPLEMENTED USERNAME LOGIN**: Removed the local passcode lock screen (`LockScreen.tsx`) and passcode autolock timeout settings from the app (`SettingsPanel.tsx`, `WorkspaceBoard.tsx`, `AiWorkspacePanel.tsx`, and `App.tsx`). Added a custom `username` field during account registration (sign up) and updated sign in/password reset to support authentication using either username or email. Implemented public `/usernames/{username}` lookup mapping in Firestore and secured it in `firestore.rules`. Verified that strict typechecking compiles cleanly with zero errors.
