...
- 2026-06-22: **OCR DESTINATION CATEGORY PICKER + CREATE CATEGORY COMPLETE**. Added full destination category picker in `OcrModal` using `CategoryPicker` + `ModalShell`. "Choose Category" opens picker, "Create Category" opens `TextPromptModal` for root category, "Create Subcategory" available when destination selected. `onCreateRootCategory`/`onCreateSubcategory` wired from App.tsx (via `createEditorRootCategory`/`createEditorSubcategory`). Destination pre-fills from `defaultDestinationPath`. `OcrResultEditor` shows destination path + "Choose" + "New" (root) + "Sub" (subcategory) buttons. Save disabled until text + destination present. Typecheck clean. Git workflow executed.
