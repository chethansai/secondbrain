...
- 2026-06-11: **ANDROID 14+ FOREGROUND SERVICE CRASH FIXED**. Root cause: TeleprompterService.kt:49 and OverlayService.kt:63 called startForeground(id, notification) without required foregroundServiceType parameter on Android 14+ (API 34+). Manifest declares foregroundServiceType="specialUse" for both services, violating Android 14+ requirement that startForeground MUST include matching type. Fixed by adding conditional: if (Build.VERSION.SDK_INT >= 34) startForeground(id, notif, ServiceInfo.FOREGROUND_SERVICE_TYPE_SPECIAL_USE) else startForeground(id, notif). Crash "Native Note Taking closed because this app has a bug" resolved. Git workflow executed.
